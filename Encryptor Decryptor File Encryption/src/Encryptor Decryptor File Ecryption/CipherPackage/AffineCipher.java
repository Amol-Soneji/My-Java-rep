...
public class AffineCipher extends SubstitutionCipher {
	...
	protected String encrypt() 
	{
		for(int index = 0; index < plainText.length(); index++) {
			...
			charCode = ((super.getKey().getKeyVal() * charCode) + super.getKey().getArbitraryB()) % 1112064;
	...
	protected String decrypt() 
	{
		for(int index = 0; index < cipherText.length(); index++) {
			...
			charCode = (super.getKey().getAffineDecKey() * (charCode - super.getKey().getArbitraryB())) % 1112064;
